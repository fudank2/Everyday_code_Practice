...
ll binaryroot(ll left,ll right,ll number)
{
    if(number==0||number==1)return number;

    ll mid=left+right;
    long long gnumber= (long long)mid*mid;
    if(gnumber<=number&&(long long)(mid+1)*(mid+1)>number)return mid;
    else if(gnumber<number)
    {
        return binaryroot(mid+1,right,number);
    }
    else{
        return binaryroot(left,mid-1,number);
    }
}
