...
void checkstring(vector<vector<char>>grid,int n,int m,int i,int j,int k,string s)
{
    ...
    if(grid[i][j]==s[k])
    {
        ...
        {
            dp[i][j-1]=1;
            checkstring(grid,n,m,i,j-1,k+1,s);
            dp[i][j-1]=0;
        }
        if(dp[i-1][j]!=1&&i>0)
        {
            dp[i-1][j]=1;
            checkstring(grid,n,m,i-1,j,k+1,s);
            dp[i-1][j]=0;
        }
        if(dp[i][j+1]!=1&&j<m)
        {
    ...
    {
        checkstring(grid,n,m,i,j-1,k,s);
        checkstring(grid,n,m,i-1,j,k,s);
        checkstring(grid,n,m,i,j+1,k,s);
        checkstring(grid,n,m,i+1,j,k,s);
    }
