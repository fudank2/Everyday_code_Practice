...
bool searchinrotae(vector<int>nums,int target,int left,int right)
{
    ...
    {
        if(target>=nums[left]&&target<=nums[mid])return searchinrotae(nums,target,left,mid-1);
        else return searchinrotae(nums,target,mid+1,right);
    }
    else
    {
        if(target>=nums[mid]&&target<=nums[right])return searchinrotae(nums,target,mid+1,right);
        else return searchinrotae(nums,target,left,mid-1);
    }
    return false;
}
